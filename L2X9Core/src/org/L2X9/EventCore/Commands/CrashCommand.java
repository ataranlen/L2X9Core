					...
					if (args[0].equalsIgnoreCase("elytra")) {
						...
						if (args[0].equalsIgnoreCase("everyone")) {
							...
							if (Bukkit.getOfflinePlayer(args[0]).isOnline()) {
								...
								if (!args[0].equalsIgnoreCase("nearby") || args[0].equalsIgnoreCase("taco") || args[0].equalsIgnoreCase("everyone") || args[0].equalsIgnoreCase("elytra")) {
							...
							try {
								if (args[0].equalsIgnoreCase("nearby")) {
									if (sender instanceof Player) {
										...
										for (Player nearby : cmdSender.getLocation()
												.getNearbyPlayers(Integer.parseInt(args[1]))) {
											if (!nearby.hasPermission("l2x9Core.crash")) {
												Utils.crashPlayer(nearby);
												Utils.sendMessage(cmdSender, "&6You have just crashed&r&c " + nearby.getName());
											}
										}

									} else {
										Utils.sendMessage(sender, "&4Error:&r&c You must be a player to use /crash nearby");
									}
								}else {
