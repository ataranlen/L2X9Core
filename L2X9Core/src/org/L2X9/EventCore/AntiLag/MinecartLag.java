...
public class MinecartLag implements Listener {
	...
	public void onSpawn(VehicleCreateEvent event) {
		...
		if (ammount >= max) {
			...
			Utils.sendOpMessgge(Utils.getPrefix() + "&6Removed &r&3" + chunk.getEntities().length + " " + formattedName + "&r&6 from a lag machine owned by&r&3 " + player.getName());
	...
	public void onVehicleMove(VehicleMoveEvent event) {
		...
		if (!event.getFrom().getChunk().equals(event.getTo().getChunk())) {
			if (chunk.getEntities().length >= max) {
				vehicle.remove();
				Utils.sendMessage(player, Utils.getPrefix() + "&6Please limit " + formattedName + " to &r&c" + max + "&r&6 per chunk");
				Utils.sendOpMessgge(Utils.getPrefix() + "&6Deleted a &r&3" + formattedName1 + "&r&6 from a lag machine owned by&r&3 " + player.getName() + " &4BYPASS ATTEMPT");
				System.out.println(ChatColor.translateAlternateColorCodes('&', Utils.getPrefix() + "&6Deleted a &r&3" + formattedName1 + "&r&6 from a lag machine owned by&r&3 " + player.getName() + " &4BYPASS ATTEMPT"));
